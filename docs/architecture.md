...
配置统一集中在 `agents.config.Settings`，来源为 `agents.yaml` 顶层 `settings` 节。运行时配置与 Agent registry 配置合并在同一文件中，不再直接从环境变量或 `.env` 读取运行时配置。

主要配置字段包括：`app_name`、`debug`、`storage_backend`、`db_path`、`postgres_dsn`、`sessions_dir`、`default_user_id`、`api_host`、`api_port`、`api_reload` 和 `cors_*`。

CLI/API 的命令行参数可覆盖 YAML 中对应字段，例如 `--sessions-dir`、`--host`、`--port`、`--reload`、`--debug`。

YAML 值支持 `${ENV_NAME}` 形式的环境变量注入（不支持 `:-` 默认值）。`default_user_id` 默认是 `default`，这是用户隔离标识，不是认证或授权机制。CLI 可用 `--user-id` 指定启动用户，交互中可用 `/user` 查看当前用户、`/user <id>` 切换后续消息的用户作用域。API 聊天请求可在 Body 中传入 `user_id`。
...
环境变量注入采用显式语法：`${VAR}`（不支持 `:-default`）。LLM 的 `model`、`api_key`、`base_url` 以及 Tool `config` 中的字符串值支持这种语法。API Key 允许明文配置，但日志、错误信息、CLI 输出、API 响应和 SSE 事件流不得输出 secret 明文。
...
对话历史库是应用级共享数据库，默认使用 SQLite，位于 `./data/agent.sqlite3`；也可通过 `agents.yaml` 的 `settings.storage_backend: postgres` 与 `settings.postgres_dsn` 切换到 PostgreSQL。
