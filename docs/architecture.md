...
SSE 接口复用 `AgentService.chat_stream()`，因此与 CLI 使用同一套事件归一化逻辑。API 会过滤 `internal_state`，不会把 DeepAgents/LangGraph 的内部状态同步事件暴露给前端。API 启动时加载完整 Agent registry，请求体不支持切换 Agent；旧 `/chat` 与 `/chat/stream` 使用启动参数中的默认 Agent，新 `/agents/{agent_name}/...` 路由通过 URL 选择 Agent。聊天请求通过 Body 字段 `user_id` 区分用户作用域，未传时使用默认用户 `default`。跨域访问通过 FastAPI `CORSMiddleware` 实现，只有在 YAML `settings.cors_origins` 中配置 origin 后才启用 CORS。
...
4. 组装本轮输入
5. 流式调用 `DeepAgents`
6. 将用户消息、助手输出、思考/步骤、工具调用和工具结果归一为事件
7. 写入当前用户作用域的结构化消息事件
8. 返回结构化结果
...
- skills 目录
- memory 文件列表
- 受限 session backend，并按 `user_id` + `thread_id` 使用 `data/sessions/<user-id>/<session-id>` 隔离文件工具、skills 和 memory
- 线程 ID 透传；传给 DeepAgents/LangGraph 的 runtime thread key 会包含用户和会话，避免不同用户同名会话共享运行时状态
...
- `agents.core.definitions`：`agents.yaml` 配置模型、`${VAR}` 引用解析、引用校验、Tool Provider 实例化和 Agent registry 合并。
...
配置统一集中在 `agents.yaml`。顶层 `settings:` 使用 `agents.config.Settings` 字段名，例如 `openai_base_url`、`storage_backend`、`sessions_dir`。代码不会直接读取 `.env`、扫描环境变量或收集自定义环境变量；只有 YAML 字段值显式写成 `${VAR}` 或 `${VAR:-default}` 时，loader 才解析该单个引用。

主要配置范围：

- 应用与日志：`app_name`、`debug`
- LLM 默认值：`model`、`openai_api_key`、`openai_base_url`
- 存储：`storage_backend`、`db_path`、`postgres_dsn`
- 会话文件：`sessions_dir`、`default_user_id`
- API：`api_host`、`api_port`、`api_reload`
- CORS：`cors_origins`、`cors_allow_credentials`、`cors_allow_methods`、`cors_allow_headers`

`default_user_id` 默认是 `default`。这是用户隔离标识，不是认证或授权机制。CLI 可用 `--user-id` 指定启动用户，交互中可用 `/user` 查看当前用户、`/user <id>` 切换后续消息的用户作用域。API 聊天请求可在 Body 中传入 `user_id`。
...
- `settings` 定义 runtime 配置，与 `Settings` 字段一致；字符串值支持 `${VAR}` 和 `${VAR:-default}`，`db_path` 与 `sessions_dir` 的相对路径按 YAML 文件所在目录解析。
...
YAML 引用采用显式语法：`${VAR}` 或 `${VAR:-default}`。YAML `settings`、LLM 的 `model`、`api_key`、`base_url` 以及 Tool `config` 中的字符串值支持这种语法。API Key 允许明文配置，但日志、错误信息、CLI 输出、API 响应和 SSE 事件流不得输出 secret 明文。推荐只在密钥或 DSN 这类敏感字段中使用 `${VAR}`，其他配置直接写 YAML 值。
...
- `threads`：会话线程，按 `user_id` + `thread_id` 形成用户作用域
- `messages`：对话消息和事件，记录 `user_id` 和 `thread_id`，使用 `message_type` 区分 `user_message`、`assistant_message`、`thinking`、`tool_call`、`tool_result` 等类型；DeepAgents/LangGraph 的内部状态更新如 middleware 生命周期事件和 `Overwrite(...)` 不作为可见对话事件持久化，工具调用分片和重复工具结果会在事件层过滤
- `memory_entries`：应用侧登记的长期记忆文件

对话历史库是应用级共享数据库，默认使用 SQLite，位于 `./data/agent.sqlite3`；也可通过 YAML `settings.storage_backend=postgres` 与 `settings.postgres_dsn` 切换到 PostgreSQL。
...
每个会话按照 `user_id` 和 `thread_id` 分到 `data/sessions/<user-id>/<session-id>`，并拥有独立的 `skills/` 和 `memory/`。CLI 的 `--new` 会生成随机 `thread_id` 来开启新 session。DeepAgents 文件 backend 的根目录就是当前 session 目录，运行期临时文件、中间结果、缓存和生成草稿直接写入该 session 根目录。

旧数据库记录没有 `user_id` 时会回填到默认用户 `default`。旧的单层 session 目录不会作为跨用户共享目录继续使用；新访问会按双层 `<user-id>/<session-id>` 结构创建目录。
