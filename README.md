...
- `src/agents/core/definitions.py`
  - `agents.yaml` 配置模型、`${VAR}` 引用解析、引用校验和 Agent registry loader
...
运行时配置统一来自 `agents.yaml`。代码不会直接读取 `.env` 或扫描环境变量；只有 YAML 字段值显式写成 `${VAR}` 或 `${VAR:-default}` 时，loader 才会解析该单个引用。推荐把所有非密钥配置直接写在 YAML 中，只对 API Key、DSN 等敏感值使用 `${VAR}` 引用。

`settings:` 字段名与 `agents.config.Settings` 一致：

| YAML 字段 | 默认值 | 说明 |
| --- | --- | --- |
| `app_name` | `agents` | 应用名称，主要用于日志。 |
| `debug` | `false` | 启动时是否开启 debug 日志。交互式 CLI 中仍可用 `/debug on|off` 临时切换。 |
| `model` | `openai:gpt-4.1` | 默认 LLM 模型。`openai:<model>` 会在请求 provider 时去掉 `openai:` 前缀。 |
| `openai_api_key` | 空 | OpenAI-compatible API key。 |
| `openai_base_url` | `https://api.openai.com/v1` | OpenAI-compatible base URL。 |
| `storage_backend` | `sqlite` | 存储后端。支持 `sqlite`、`postgres`、`postgresql`、`pg`。 |
| `db_path` | `./data/agent.sqlite3` | SQLite 数据库路径。相对路径按 YAML 文件所在目录解析。 |
| `postgres_dsn` | 空 | PostgreSQL DSN。 |
| `sessions_dir` | `./data/sessions` | 会话文件根目录。相对路径按 YAML 文件所在目录解析。 |
| `default_user_id` | `default` | 未显式指定用户时使用的默认用户隔离标识。 |
| `api_host` | `127.0.0.1` | API 启动监听 host。 |
| `api_port` | `8000` | API 启动监听 port。 |
| `api_reload` | `false` | API 启动时是否启用 reload。 |
| `cors_origins` | 空 | API CORS 允许的 origin，逗号分隔。为空时不启用 CORS；本地开发可设为 `*`。 |
| `cors_allow_credentials` | `false` | CORS 是否允许 credentials。设为 `true` 时不要把 origins 配成 `*`，应明确列出前端域名。 |
| `cors_allow_methods` | `*` | CORS 允许的方法，逗号分隔。 |
| `cors_allow_headers` | `*` | CORS 允许的请求头，逗号分隔。 |
...
默认数据目录是当前目录下的 `data/`。SQLite 对话历史是全局共享数据库，默认是 `./data/agent.sqlite3`；PostgreSQL 使用 `settings.postgres_dsn`。每个对话会按 `user_id` 和 `thread_id` 在 `./data/sessions/` 下创建独立文件目录，里面包含该用户会话自己的 `skills/` 和 `memory/`。`user_id` 是隔离标识，不是认证或授权机制；未指定时使用 `settings.default_user_id`。
...
默认不会显示中间请求日志，只显示交互内容。启动时是否开启 debug 日志由 YAML `settings.debug` 控制；交互模式中也可以动态开关：
...
YAML 顶层包含可选 `settings`，以及必需的 `llms`、`tools`、`agents`：
...
- `settings` 对应 `agents.config.Settings` 字段，是运行时配置的唯一来源；字段值可直接写 YAML 值，也可写 `${VAR}` 或 `${VAR:-default}` 引用。`db_path` 和 `sessions_dir` 使用相对路径时，按 YAML 文件所在目录解析。
- `llms` 定义 LLM 资源，`model` 保持 DeepAgents 的 `provider:model` 格式；当前 OpenAI-compatible 路径仍强制使用 Chat Completions。
- `model`、`api_key`、`base_url` 支持 `${VAR}` 和 `${VAR:-default}` 引用。
- `tools` 定义 Tool Provider，`provider` 是完整类路径，`config` 会作为关键字参数传给 provider 构造函数；`config` 内字符串也支持 `${VAR}` 引用。
...
- API Key 允许明文配置，但日志、错误信息、CLI 输出、API 响应和 SSE 事件流都不会输出 secret 明文。推荐只在 YAML 中对密钥字段使用 `${VAR}` 引用，其他配置直接写 YAML 值。
...
```bash
agents-api --agent default --agent-config ./agents.yaml
python -m agents.interfaces.api --agent default --agent-config ./agents.yaml
```
...
5. 在 `agents.yaml` 或 `--agent-config` 指向的 YAML 中配置 `settings`、`llms`、`tools` 和 `agents`。
6. `Parameters` 可选：
   - 连续对话：留空（等价 `agents`）
   - 新会话：`--new`
...
```bash
uvicorn agents.interfaces.api:app
```
...
4. `Working directory` 设为项目根目录。
5. `Parameters` 可选：`--agent default --agent-config ./agents.yaml`。
6. API host、port、reload 和 CORS 均在 YAML `settings:` 中配置。
...
浏览器跨域访问需要在 YAML `settings.cors_origins` 中配置 CORS，例如：
...
不配置 `cors_origins` 时，API 不启用 CORS。
